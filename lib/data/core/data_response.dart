...
extension DataResponseStreamExt<T> on Stream<DataResponse<T>> {
  ...
  Stream<DataResponse<C>> mapSuccess<C>(C Function(T data) onSuccess) async* {
    await for (final DataResponse<T> response in this) {
      if (response is Success) {
  ...
  Stream<DataResponse<C>> mapDataResponse<C>(
    DataResponse<C> Function(T data) onSuccess,
  ) async* {
    await for (final DataResponse<T> response in this) {
      if (response is Success) {
        yield onSuccess.call((response as Success).data);
      } else if (response is Error) {
        yield Error((response as Error<T>).error);
      } else {
        yield InProgress();
